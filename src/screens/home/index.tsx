                ...
                <div className="network__item" key={x.name}>
                  ...
                  <div className="network__item--links">
                    {x.mainnet.map((network) => (
                      <NetworkLink className="mainnet" network={network} key={network.chainId} />
                    ))}
                    {x.testnet.map((network) => (
                      <NetworkLink className="testnet" network={network} key={network.chainId} />
                    ))}
                    {x.retired.map((network) => (
                      <NetworkLink className="retired" network={network} key={network.chainId} />
                    ))}
                    {x.other.map((network) => (
                      <NetworkLink className="other" network={network} key={network.chainId} />
                    ))}
                  </div>
                </div>
              );
